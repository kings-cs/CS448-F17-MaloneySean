...
/**
 * A class that allows us to work with a knowledge base representing the Wumpus World.
 * 
 * @author Chad Hogg
 * @version 2017
 */
public class WumpusWorldAgent {
		...
		while(!isPlayerDead && !didPlayerQuit) {
			...
			if(typed.equals("y")) {
				...
				while(column < 0 || column > GRID_SIZE - 1) {
					System.out.println("Please try again.  Column must be in the range (" + 0 + ", " + (GRID_SIZE - 1) + ") inclusive: ");
					column = input.nextInt();
				}
				System.out.print("Which row are you standing in?: ");
				int row = input.nextInt();
				while(row < 0 || row > GRID_SIZE - 1) {
					System.out.println("Please try again.  Row must be in the range (" + 0 + ", " + (GRID_SIZE - 1) + ") inclusive: ");
					row = input.nextInt();
				}
	 ...
	 * 
	 * @param algorithm An algorithm to use for checking entailment.
	 * @param knowledgeBase That which we know for sure.
	 * @param column The column.
	 * @param row The row.
	 */
	public static void printKnowledgeAboutSquare(EntailmentChecker algorithm, Sentence knowledgeBase, int column, int row) {
	...
	/**
	 * Gets all of the knowledge rules that are true in every instance of Wumpus World, before the player has observed anything.
	 * 
	...
	public static Conjunction getInitialKnowledgeBase() {
			...
			for(int j = 0; j < GRID_SIZE; j++) {
				for(int k = 0; k < GRID_SIZE; k++) {
					for(int l = 0; l < GRID_SIZE; l++) {
						if(i != k && j != l) {
							knowledgeBase.addSentence(Sentence.parse("( " + buildWumpusAtString(i, j) + " --> ! ( " + buildWumpusAtString(k, l) + " ) )"));
						}
					}
				}
			}
		...
		// If you have visited a non-smelly square, neither it nor any adjacent squares have a wumpus.
		for(int i = 0; i < GRID_SIZE; i++) {
			for(int j = 0; j < GRID_SIZE; j++) {
		...
		// If you have visited a non-breezy square, neither it nor any adjacent squares have a pit.
		for(int i = 0; i < GRID_SIZE; i++) {
			for(int j = 0; j < GRID_SIZE; j++) {
				Conjunction antecedent = new Conjunction();
				antecedent.addSentence(new Proposition(buildVisitedString(i, j)));
		...
		// If a square is breezy, then at least one adjacent square or it is a pit.
		for(int i = 0; i < GRID_SIZE; i++) {
			for(int j = 0; j < GRID_SIZE; j++) {
		...
		// If a square is smelly, then at least one adjacent square or it contains the wumpus.
		for(int i = 0; i < GRID_SIZE; i++) {
			for(int j = 0; j < GRID_SIZE; j++) {
		...
		// It is safe to enter a square if it does not have a pit and either does not have a wumpus or the wumpus is dead.
		for(int i = 0; i < GRID_SIZE; i++) {
			for(int j = 0; j < GRID_SIZE; j++) {
				knowledgeBase.addSentence(Sentence.parse("( " + buildSafeToEnterString(i, j) + " <-> ( ! ( " + buildPitAtString(i, j) + " ) && ( " + buildWumpusDeadString() + " || ! ( " + buildWumpusAtString(i , j) + " ) ) ) )"));
			}
		}
		...
		for(int i = 0; i < GRID_SIZE; i++) {
			for(int j = 0; j < GRID_SIZE; j++) {
				knowledgeBase.addSentence(Sentence.parse("( ( " + buildVisitedString(i, j) + " && " + buildPitAtString(i, j) + " ) --> " + buildPlayerDeadString() + " )"));
				knowledgeBase.addSentence(Sentence.parse("( ( " + buildVisitedString(i, j) + " && " + buildWumpusAtString(i, j) + " && ! ( " + buildWumpusDeadString() + " ) ) --> " + buildPlayerDeadString() + " )"));
			}
		}
	...
	/**
	 * Gets the String for a Proposition saying that the wumpus is at a given location.
	 * 
	...
	/**
	 * Gets the String for a Proposition saying that it is smelly at a given location.
	 * 
	 * @param column Which column.
	 * @param row Which row.
	 * @return A String for that Proposition.
	...
	/**
	 * Gets the String for a Proposition saying that there is a pit at a given location.
	 * 
	 * @param column Which column.
	 * @param row Which row.
	 * @return A String for that Proposition.
	...
	/**
	 * Gets the String for a Proposition saying that it is breezy at a given location.
	 * 
	 * @param column Which column.
	 * @param row Which row.
	 * @return A String for that Proposition.
	...
	/**
	 * Gets the String for a Proposition saying that you have visited a given location.
	 * 
	 * @param column Which column.
	 * @param row Which row.
	 * @return A String for that Proposition.
	...
	/**
	 * Gets the String for a Proposition saying that it is safe to enter a given location.
	 * 
	 * @param column Which column.
	 * @param row Which row.
	 * @return A String for that Proposition.
	...
	/**
	 * Gets the String for a Proposition saying that the player has the gold in his possession.
	 * 
	 * @return A String for that Proposition.
	 */
